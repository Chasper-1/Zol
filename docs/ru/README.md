...
```
Zol/
  Cargo.toml              ← workspace (5 крейтов)
  crates/
    zoll/                 ← Парсер разметки zoll (не зависит от всего)
      src/
        token/            ← Токенизатор (один проход)
        parser/           ← Стековый построитель AST
        ast/              ← AST-узлы, маркеры, стили
        lib.rs            ← Публичное API: parse_document()

    editor/               ← Ядро редактора (независимо от GUI)
      src/
        cursor/           ← Курсор с grapheme-навигацией
