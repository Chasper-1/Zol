...
// ⚠
// ⚠  - Не клонируй ShapedDocument (не делай .clone()).
// ⚠    buffer + line_runs = огромные данные.
// ⚠
// ⚠  - Не делай reshape на каждый кадр без необходимости.
// ⚠    Проверяй doc.dirty перед вызовом (сейчас так и есть — не ломай).
// ⚠
// ⚠  - Не храни RefMut<'_, ShapedDocument> между вызовами —
// ⚠    это удерживает заимствование и может вызвать panic.
// ⚠
// ⚠  - Не удерживай borrow на doc.contents() одновременно с borrow
// ⚠    на shaped_doc — reverse borrow panics в рантайме.
// ⚠
// ⚠  Причина: clone ShapedDocument копирует ВЕСЬ buffer (cosmic-text)
// ⚠  + все line_runs (текст документа) — это 2x памяти на каждый клон.
// ═══════════════════════════════════════════════════════════════════════
...
fn draw_reshape(this: &IcedEditor<'_>, bounds: Rectangle) {
    ...
    if needs_reshape {
        ...
        let cursor_raw = Some(doc.cursor.raw());
        let cursor_line = Some(doc.cursor.line());
